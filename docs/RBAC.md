...
- **Admin > Hệ thống > Giám sát Queue** (`admin.queue.*`, gate `manage-queue`) — `App\Backend\Controllers\QueueMonitorController`. Custom-built page (not a third-party package — see "Queue monitoring" in `docs/DOCKER.md` for why): per-queue pending/reserved/delayed counts (auto-refreshing via polling `/admin/queue/stats`), paginated failed-jobs list with retry/delete/retry-all actions.
