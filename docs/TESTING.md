...
| `queueMonitor` | Custom queue dashboard: `manage-queue` gate (guest denied, no-permission user denied, permitted user allowed), per-queue Redis stats via `Queue::connection('redis')`, `failed_jobs` retry/delete/retry-all (incl. 404 for unknown uuid) | `tests/Feature/Backend/Controllers/QueueMonitorControllerTest.php` |
