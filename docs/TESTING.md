...
| `aiFeatures` | AI prediction + chat: default models are current (no retired ones) and overridable via `GROQ_MODELS`, fall-through past retired/empty models, `<think>` stripped, missing key fails fast, request carries key/model/today's date, failed predictions never cached (next click succeeds) while real ones are, `/ai-predict` + `/ai-chat` return 503 + message (not an error disguised as an answer) in the user's language, chat validation, prediction prompt carries the real market snapshot (and survives a missing/broken one), system prompt forbids inventing figures, chat popup has no inline handlers and the ids the module binds to; `Node`: `formatAiText` (escaping/XSS, bold/lists/headings/tables, `<br>` in cells) | `tests/Feature/Frontend/AiFeaturesTest.php`, `tests/js/ai-text.test.mjs` |
| `dbBackup` | Weekly DB backup: `year/month/day` layout without zero padding, zip holds the `.sql`, named after the DB, no temp leftovers, default root beside the project / configured root, freshness (valid zip < 7 days; broken/empty zips ignored; newest wins), truncated/empty dump and failing mysqldump rejected, previous backup survives a failed run, command skip / backup / `--force` / error exit / `--list`, hourly schedule + entrypoint order (mysqldump itself is stubbed) | `tests/Feature/Support/DatabaseBackupTest.php` |
