...
| `queueMonitor` | Horizon dashboard authorization (`Horizon::auth()` in `HorizonServiceProvider`) — guest denied, `manage-queue` permission required and sufficient on its own (not just "any backend permission") | `tests/Feature/Providers/HorizonServiceProviderTest.php` |
